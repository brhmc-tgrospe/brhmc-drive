<template>
  <div class="fixed inset-0 z-[70] flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl max-w-sm w-full p-5 sm:p-6 text-center animate-fade-in-up">
      ...
      <h3 class="text-lg sm:text-xl font-bold text-slate-800 mb-2">Delete Checklist(s)?</h3>
      <p class="text-xs sm:text-sm text-slate-500 mb-4 sm:mb-6">
        Are you absolutely sure you want to permanently remove 
        <span class="font-bold text-slate-700">{{ deleteCount }}</span> checklist(s)? This action cannot be undone.
      </p>
...
const deleteCount = computed(() => props.checklist ? 1 : props.batchIds.length);
...
const executeDelete = async () => {
    ...
    try {
        if (props.checklist) {
            // Single Delete
            await checklistStore.deleteChecklist(props.checklist.id);
            toastStore.show(`Checklist #${props.checklist.id} deleted.`, 'success');
        } else {
            // Batch Delete
            await checklistStore.batchDeleteChecklists(props.batchIds);
            toastStore.show(`${props.batchIds.length} checklists deleted.`, 'success');
        }
        
        emit('deleted');
    } catch (e) {
        console.error("Delete Error:", e);
        toastStore.show(e.response?.data?.message || 'Failed to delete checklist(s). Server may be blocking the request.', 'error');
    } finally {
