    ...
    <div class="relative bg-white rounded-lg shadow-2xl w-full max-w-2xl flex flex-col max-h-[95vh] sm:max-h-[90vh]">
      ...
      <div class="p-4 sm:p-6 overflow-y-auto flex-1">
        <form @submit.prevent="saveUser" class="space-y-3 sm:space-y-4">
          ...
          <div>
            <label class="block text-[10px] sm:text-xs font-bold text-slate-600 mb-1">Password <span class="text-slate-400 font-normal">(Leave blank to keep current)</span></label>
            <input v-model="form.password" type="password" minlength="6" class="w-full bg-slate-50 border border-slate-200 rounded px-2 py-1.5 sm:px-3 sm:py-2 text-xs sm:text-sm focus:ring-2 focus:ring-teal-500 outline-none">
          </div>

          <div class="pt-2 border-t border-slate-100">
            <label class="block text-[10px] sm:text-xs font-bold text-slate-800 mb-1">Job Title (Role)</label>
            <input v-model="form.role" type="text" disabled class="w-full bg-slate-100 text-slate-500 border border-slate-200 rounded px-2 py-1.5 sm:px-3 sm:py-2 text-xs sm:text-sm outline-none cursor-not-allowed">
